...
public class SQLiteHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("create table " + TB__BOOKMARK_NAME + " ( "
				...
				+ " varchar, " + HistoryBean.ISBOOKMARK + " integer, "
				+ HistoryBean.TIME + " integer)");
		db.execSQL("create table " + TB__SEARCH_NAME + " ( " + HistoryBean.NAME
	...
	public void add_history(Context context, String name, String url,
		...
		if (isbookmark == 0) {
			...
			if (temp_cursor.moveToFirst()) {
				...
				// 疑问：关于整型引号问题
				SQL = "insert into  " + TB__HISTORY_NAME + "("
						+ HistoryBean.TIME + "," + HistoryBean.NAME + ","
						+ HistoryBean.URL + "," + HistoryBean.ISBOOKMARK + ")"
						+ "values(" + time + ",'" + name + "','" + url + "',"
						+ isbookmark + ");";
				TIP = "insert";
			}
		} else {
			temp_cursor = db.rawQuery("select * from " + TB__BOOKMARK_NAME
					+ " where name=" + "'" + name + "'" + ";", null);
			if (temp_cursor.moveToFirst()) {
				SQL = "update " + TB__BOOKMARK_NAME + " set "
						+ HistoryBean.TIME + "=" + time + ","
						+ HistoryBean.ISBOOKMARK + "=" + isbookmark
						+ " where name=" + "'" + name + "';";
				TIP = "update";
			} else {
				// 疑问：关于整型引号问题
				SQL = "insert into  " + TB__BOOKMARK_NAME + "("
						+ HistoryBean.TIME + "," + HistoryBean.NAME + ","
						+ HistoryBean.URL + "," + HistoryBean.ISBOOKMARK + ")"
						+ "values(" + time + ",'" + name + "','" + url + "',"
						+ isbookmark + ");";
